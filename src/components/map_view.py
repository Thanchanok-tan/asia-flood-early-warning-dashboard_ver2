...
def render_map_view(df: pd.DataFrame, lang: str):
    """Render interactive Plotly geospatial early warning map and station inspector."""
    ...
    latest_df["hover_info"] = (
        ...
        "<span style='color:#334155;'>" + get_text(lang, "hover_severity") + ": <b>" + latest_df["severity_level"] + "</b></span><br>" +
    ...
    # ---------------------------------------------------------
    st.markdown("---")
    st.subheader(f"🔍 {get_text(lang, 'station_inspector_title')}")
    ...
    if selected_station:
        ...
        lat_val = latest_row["latitude"]
        lon_val = latest_row["longitude"]

        gauge_m = latest_row["river_level_m"]
        rain_mm = latest_row["rainfall_mm"]
        soil_pct = latest_row["soil_moisture_percent"]
        temp_c = latest_row["temperature_c"]

        risk_score = latest_row["flood_risk_score"]
        prob_pct = round(risk_score * 100, 1)
        severity = latest_row["severity_level"]
        ...
        if len(station_ts) > 1:
            prev_gauge = station_ts.iloc[-2]["river_level_m"]
        ...
        recent_ts = station_ts.tail(90)  # Show last 90 observations
